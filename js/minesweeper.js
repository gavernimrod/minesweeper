...
const COVER_HELP = '<src="img/cover2.png"/>';
...
function removeInitNegsFromEmptyCells() {
  for (var idx = 0; idx < gInitNegs.length; idx++) {
...
function cellClicked(i, j) {
  ...
  if (cell.isMine && !gHintModeOn && gGame.isOn) {
    showCell(i, j);
    gShowMines = setInterval(showAllMines, 100);
    gameOver("deadMine");
    // declareDead()
  }
  // toggleSmiley();
...
function showSafeCell() {
  ...
  var safeCellIndex = getRandomInt(0, hiddenCells.length - 1)
  var safeCell = hiddenCells[safeCellIndex]

  var elCell = document.querySelector(`#cover-${safeCell.i}-${safeCell.j}`)
  elCell.classList.add('hide');
  elCell.classList.remove('show');
  gSafeLeft--
  document.querySelector('#safer').innerText = `You got ${gSafeLeft} safe clicks left`
  setTimeout(() => {
